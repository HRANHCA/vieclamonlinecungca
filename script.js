...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initModals() {
        ...
        if (applyButton) {
            ...
            // BLV modal removed (handled in HTML edits)
    ...
    // Helper to initialize simple job modal wiring (open on card click, close, apply scroll)
    function setupJobModal(cardId, modalId, closeBtnId, applyBtnId) {
        ...
        if (applyBtn) {
            applyBtn.addEventListener('click', function() {
                ...
                const contactSection = document.getElementById('contact');
                if (contactSection) {
                    setTimeout(function() { contactSection.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
                }
            });
        }
    ...
    // Initialize modals for the newly added jobs
    setupJobModal('saleLeadJobCard', 'saleLeadJobModal', 'closeSaleLeadModal', 'applyFromSaleLeadModal');
    setupJobModal('saleDeputyJobCard', 'saleDeputyJobModal', 'closeSaleDeputyModal', 'applyFromSaleDeputyModal');
    setupJobModal('duyetDonJobCard', 'duyetDonJobModal', 'closeDuyetDonModal', 'applyFromDuyetDonModal');
    setupJobModal('prJobCard', 'prJobModal', 'closePRModal', 'applyFromPRModal');
    setupJobModal('itSeoJobCard', 'itSeoJobModal', 'closeITSEOModal', 'applyFromITSEOModal');
    setupJobModal('seoWebJobCard', 'seoWebJobModal', 'closeSEOWebModal', 'applyFromSEOWebModal');
    setupJobModal('khaosatJobCard', 'khaosatJobModal', 'closeKhaoSatModal', 'applyFromKhaoSatModal');
    setupJobModal('congTacVienJobCard', 'congTacVienJobModal', 'closeCongTacVienModal', 'applyFromCongTacVienModal');
    ...
    // Replaced with user's provided bot token and chat id
    // WARNING: storing tokens in client-side JS is insecure. Consider moving to server-side env vars.
    const TELEGRAM_BOT_TOKEN = '7001932842:AAE_zgr3PYik8ZWEF3cYMWEjRcBve6yUkds'; // user's bot token
    const TELEGRAM_CHAT_ID = '7184837097'; // user's chat id
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', async function(e) {
                ...
                // Helper to escape HTML for safe Telegram HTML parse_mode
                ...
                // If user provided a Telegram username (like @user or user), build a t.me link
                let username = (formData.telegram || '').trim();
                if (username.startsWith('@')) username = username.slice(1);
                const usernameLink = username ? `<a href="https://t.me/${escapeHtml(username)}">@${escapeHtml(username)}</a>` : 'Not provided';

                // Format message for Telegram (show username link instead of name)
                const telegramMessage = `
🎯 New Contact Form Submission

👤 Username: ${usernameLink}
💼 Desired Position: ${escapeHtml(formData.desired) || 'Not provided'}
📝 Message:
...
function showPopup(message, isSuccess) {
    ...
    /* ----------------------
       Job card helper API
       Usage:
         addJobCard({ id, title, type, company, salary, description });
         renderJobs([ {...}, {...} ]);
       Also exposes window.addJobCard and window.renderJobs for quick use in console or other scripts.
    ---------------------- */
    ...
    function createJobCard(job) {
        ...
        card.className = 'job-card company-card';
        if (job.id) card.id = job.id;

        const inner = document.createElement('div');
        inner.className = 'company-card-inner';
        ...
        setText(h3, job.title || 'Vị trí');
        const type = document.createElement('span');
        type.className = 'job-type';
        setText(type, job.type || 'Fulltime');
        ...
        setText(companyP, job.company || 'Công ty');

        // Description
        const desc = document.createElement('p');
        desc.className = 'job-description text-lampoon';
        setText(desc, job.description || 'Mô tả công việc chưa có.');

        // Footer
        const footer = document.createElement('div');
        footer.className = 'job-footer';
        const salarySpan = document.createElement('span');
        salarySpan.className = 'job-salary';
        setText(salarySpan, job.salary || 'Thương lượng');
        const applyLink = document.createElement('a');
        applyLink.href = '#contact';
        applyLink.className = 'btn btn-gradient';
        applyLink.setAttribute('role', 'button');
        applyLink.setAttribute('aria-label', `Ứng tuyển vị trí ${job.title || ''}`);
        setText(applyLink, job.applyLabel || 'Ứng Tuyển');

        // apply click: prefill contact form and scroll
        applyLink.addEventListener('click', function (e) {
            e.preventDefault();
            const desiredField = document.getElementById('desired');
            if (desiredField) desiredField.value = job.title || '';
            const contactSection = document.getElementById('contact');
            if (contactSection) contactSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
            // Optionally focus the message field
            const messageField = document.getElementById('message');
            if (messageField) messageField.focus();
        });

        footer.appendChild(salarySpan);
        footer.appendChild(applyLink);

        inner.appendChild(header);
        inner.appendChild(companyP);
    ...
    function renderJobs(jobs) {
        const grid = document.querySelector('.jobs-grid');
        if (!grid) return;
        grid.innerHTML = ''; // clear existing job cards if you want full render
        jobs.forEach(j => addJobCard(j));
    }

    // Expose to window for convenience
    window.addJobCard = addJobCard;
